...
def _dict_to_config(data: dict) -> Config:
    ...
    return Config(
        ...
        segmentation=SegmentationConfig(
            ...
            erosion_iterations=segmentation_data.get("erosion_iterations", 0),
            crop_n_layers=segmentation_data.get("crop_n_layers", 0),
        ),
        merge=MergeConfig(
            merge_strategy=merge_data.get("merge_strategy", "best_match"),
...
def _config_to_dict(config: Config) -> dict:
    ...
    return {
        ...
        "segmentation": {
            ...
            "crop_n_layers": config.segmentation.crop_n_layers,
        },
        "merge": {
            "merge_strategy": config.merge.merge_strategy,
